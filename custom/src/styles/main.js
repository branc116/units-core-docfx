import * as ass from "https://pagead2.googlesyndication.com/pagead/js/adsbygoogle.js";
...
if (!document.querySelectorAll(".inheritedMembers div").length) {
    const el = document.querySelector(".inheritedMembers");
    if (el)
        el.remove();
}
...
/** 
* @param {string} str */
jQuery.prototype.css2 = function(str) {
    const arr = str.trim().split('\n');
    const _this = this;
    arr.forEach(i => {
        const [key, val] = i.split(":");
        _this.css(key, val.replace(";", ""));
    })
}
document.querySelectorAll(".script").forEach(i => {
    const html = i.outerHTML;
    const parent = $(i.parentElement);
    const link = i.innerText;
    const click = `"navigator.clipboard.writeText(` + '`' + link + '`)"';
    parent.prepend($(`<a onclick=${click}>${html}</a>`))
    i.remove();
});

const hashMap = {};
$("a[href='#startcountdown']").on("click", () => {
    const id = "i.id";
    if (hashMap[id])
        return;
    const el = $(sw);
...
bottom: 0.7em;`);
    sec.text("00");
    el.prependTo(document.getElementsByTagName("body"));
    let secs = 180;
    const idInterval = setInterval(() => {
        secs--;
        hm.text(`00:0${Math.floor(secs / 60)}`);
